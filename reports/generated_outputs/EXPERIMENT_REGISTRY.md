...
|---|---|---|---|
| `00_release_pipeline_diagnostics` | `paper_figures | release_pipeline_diagnostics | script=scripts/build_paper_figures.py | figure=05_degeneracy_sp500` | Compact summary tables for the release-pipeline degeneracy figure. | Figure definition, reported diagnostic constants, or upstream collapse audit. |
...
The audit/table folders `06_tatr_audit`, `07_replication_summary_tables`, and `08_replicability_diagnostic` are synthesis artifacts, not primary compute runs. Regenerate them only when upstream result folders change.
