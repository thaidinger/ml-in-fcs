...
This note compares the current local outputs for GOOG and ZC=F against the FTS-Diffusion paper claims. The local reference PDFs were removed from the public submission branch; use the OpenReview/proceedings links in the top-level README if the original paper is needed.
