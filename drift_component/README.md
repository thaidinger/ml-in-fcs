...
for FTS-Diffusion outputs. It is separated from the reference implementation so
the diagnostic can be run without touching the training code.
...
If this folder is copied out as a standalone component, run the same install
from the copied folder with `pip install -r requirements.txt`.
...
```bash
python drift_component/scripts/evaluate_predictable_drift.py \
  --real-csv data/asset_prices.csv \
  --synthetic-csv outputs/default/samples/generated.csv \
  --input-type close \
  --synthetic-input-type return \
  --output-dir outputs/default/drift_diagnostics
...
python drift_component/scripts/run_light_drift_experiment.py \
  ...
  --output-dir outputs/drift_diagnostics \
  --max-synthetic 5 \
  --null-reps 20
```

The standalone component can be run without the reference implementation. The
standalone sweep scripts write under `drift_component/outputs/` by default. The
submission branch also stores integrated drift outputs in
`reports/generated_outputs/05_drift_diagnostic_advantage`,
`reports/generated_outputs/06_end_to_end_drift_diagnostics`, and
`reports/generated_outputs/07_drift_incorporated_selection`.
