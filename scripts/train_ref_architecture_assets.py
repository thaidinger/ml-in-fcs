...
def expected_outputs(asset: str, k: int) -> dict[str, Path]:
    from models.load_models import get_pem_name, get_pgm_name

    return {
        "pgm_state_dict": REF / "trained_models" / get_pgm_name(state_dict=True),
        "pgm_full_model": REF / "trained_models" / get_pgm_name(state_dict=False),
        "pem_state_dict": REF / "trained_models" / get_pem_name(state_dict=True),
        "pem_full_model": REF / "trained_models" / get_pem_name(state_dict=False),
        "sisc_centroids": REF / "res" / f"sisc_{asset}_k{k}_l10-21_dba_kmpp_centroids.csv",
        "sisc_labels": REF / "res" / f"sisc_{asset}_k{k}_l10-21_dba_kmpp_labels.csv",
        "sisc_subsequences": REF / "res" / f"sisc_{asset}_k{k}_l10-21_dba_kmpp_subsequences.csv",
        "sisc_segmentation": REF / "res" / f"sisc_{asset}_k{k}_l10-21_dba_kmpp_segmentation.csv",
    }
...
def train_asset(asset: str, k: int, seed: int) -> dict[str, object]:
    ...
    missing_sisc = [rel(path) for name, path in outputs.items() if name.startswith("sisc_") and not path.exists()]
    if missing_sisc:
    ...
    missing_models = [rel(path) for name, path in outputs.items() if not name.startswith("sisc_") and not path.exists()]
    if missing_models:
