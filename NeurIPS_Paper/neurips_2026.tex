...
We make three contributions. (i) A code-level diagnosis of the released sampling pipeline (Section~\ref{sec:degeneracy}): three specific choices that drive the synthetic series to a near-degenerate linear ramp, plus the underspecifications we had to resolve. (ii) A 30-seed S\&P 500 TATR study, with the cross-asset observation that the continuous-rollout gain does not transfer to GOOG or corn futures (ZC=F), which we read as a scale coincidence. (iii) A set of mismatches between the paper's stated design and the released implementation (Section~\ref{sec:degeneracy:mismatch}), together with an empirical account of why the pattern chain collapses to a single motif. We also document a gap in reproducing the Appendix B.3 SISC toy-data targets. Throughout, we separate claims about the released artifacts from what would need the authors' checkpoints to settle.
...
\subsection{Three choices in the sampling code}
\label{sec:degeneracy:choices}

\paragraph{(1) Deterministic argmax rollout.} FTS-Diffusion rolls out the pattern-evolution module (PEM) by taking \texttt{argmax} for the next pattern and the next length, and the raw regression mean for the next magnitude, with no sampling (\texttt{fts-diffusion-ref/models/sampling.py}, lines 30, 33, 34):
\begin{lstlisting}
...
\subsection{Mismatches with the paper}
\label{sec:degeneracy:mismatch}

Beyond the sampling choices above, the released training code departs from the paper's stated design in three places.

\paragraph{(1) Duration trained as classification.} Although the paper models the next pattern as multi-class classification and the duration $\alpha$ and magnitude $\beta$ as regression, the pattern-evolution head treats duration as classification too: it emits $\texttt{n\_patterns}+\texttt{range\_length}+1$ logits and trains the length slice (one logit per duration in $10$--$21$) with a cross-entropy loss against the integer target $\text{length}-10$, exactly like the pattern (\texttt{pattern\_evolution\_module.py}, lines 77, 98, 105, 111). Only the magnitude is regressed, and with an $L_1$ loss; the $L_2$ \texttt{MSELoss} line is commented out (lines 78--79). Appendix~D.1 of the original paper allows that treating the length as classification is feasible, yet its main text describes regression.

\paragraph{(2) Magnitude $\beta$ ignored at generation.} \texttt{generate} receives only the pattern and length (\texttt{pattern\_generation\_module.py}, line 74), with an author comment that magnitude control was dropped from the denoising step (line 104). As in Section~\ref{sec:degeneracy:choices}(3), $\beta$ is reapplied only as a post-hoc multiply on top of a commented-out unit-range normalization, so it scales an arbitrary-range output rather than a unit shape.

\paragraph{(3) Deterministic rollout.} Generation rolls out the chain with the \texttt{argmax} of Section~\ref{sec:degeneracy:choices}(1), whereas the paper's Section~3.1 describes a stochastic Markov chain $Q(\cdot\,|\,\cdot)$. In fairness, the paper's Algorithm~2 also writes the transition as a deterministic map, so the contradiction is between the stochastic description and the implementation.

We also probed why that deterministic rollout collapses to one motif (\texttt{scripts/probe\_pem\_transition.py}). On the trained S\&P 500 PEM the next-pattern softmax is near-uniform: its maximum is $0.11$ to $0.17$ against a uniform $1/14\approx0.071$, peaked only thinly on the data mode $p{=}12$. So \texttt{argmax} returns $p{=}12$ for 13 of the 14 input patterns, and sweeping the length and magnitude inputs it never leaves the high-frequency SISC patterns $\{0,1,2,3,6,8,12\}$. The collapse is \texttt{argmax} acting on an undertrained, near-uniform head (the next-pattern loss carries weight $0.05$), not a learned absorbing transition; multinomial sampling from the same head would not collapse.

\subsection{Generated trajectories}
\label{sec:degeneracy:observed}

Figure~\ref{fig:degeneracy} contrasts a real S\&P 500 path with one released-pipeline synthetic path. That synthetic series is visually a straight line; a least-squares linear fit gives $R^2\approx0.99999$. Three quantitative observations accompany the figure. First, independent runs of a configuration are nearly identical, consistent with a deterministic rollout. Second, the PEM state sequence collapses (Figure~\ref{fig:degeneracy}, bottom): on S\&P 500 a single pattern is used in $99.6\%$ of segments (a fixed point), while GOOG and ZC=F settle into period-2 cycles. Third, inspecting the trained PGM decoder pattern by pattern, the decoder output is near-constant across all patterns (cross-pattern standard deviation $<0.001$): an up-trend pattern and a down-trend pattern yield essentially the same segment. Together these tile one tiny upward-creeping segment across the horizon, producing exactly the linear ramp in Figure~\ref{fig:degeneracy}. The same holds on the other assets: GOOG and ZC=F continuous rollouts are also near-linear (linear-fit $R^2\approx0.99999$), since their period-2 chains alternate between two near-identical segments.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{05_degeneracy_sp500.pdf}\\[6pt]
  \includegraphics[width=0.62\linewidth]{03_pem_collapse.pdf}
  \caption{Released-pipeline degeneracy on S\&P 500. Top, left to right: (a) real close prices show rich structure; (b) one synthetic continuous-rollout path is almost perfectly linear (linear-fit $R^2\approx0.99999$); (c) daily-return density on a log scale, where synthetic returns concentrate near a single value with standard deviation about $20\times$ smaller than real returns. Bottom: PEM state-sequence collapse under the deterministic rollout (from saved \texttt{run\_*\_states.npy}), the share of generated segments that use the single most frequent pattern. A single pattern covers $99.6\%$ of segments on S\&P 500 ($K{=}14$, a fixed point, 2520 segments) and $50\%$ on GOOG ($K{=}11$, 2520 segments) and ZC=F ($K{=}11$, 1867 segments), both period-2 cycles; the dashed line marks the uniform expectation $1/K$ a non-degenerate chain would approach. We show one representative run; runs are nearly identical.}
  \label{fig:degeneracy}
\end{figure}

\subsection{Underspecifications}
\label{sec:degeneracy:underspec}

Several choices that matter for the result are not pinned down by the paper. Table~\ref{tab:underspec} lists the ones we encountered. We kept the released values wherever they existed, so that the run reflects the artifact as published rather than our preferences. We intend this as an audit aid rather than an accusation: underspecification is common, but here it coincides with the failure modes.
...
\begin{table}[h]
  ...
  \begin{tabular}{p{5.2cm}p{1.6cm}p{5cm}}
    \toprule
    Design choice & In paper? & Value used \\
    \midrule
    Scaling-AE decoder hidden dim & no & $1$ (released) \\
    PEM rollout policy & no & argmax (released) \\
    Magnitude normalization at sampling & implied & commented out (released) \\
    PGM / PEM training epochs & no & $30$ / $60$ (ours; released $400$ / $1000$) \\
    PCDM diffusion steps & no & $30$ (ours; released $100$) \\
    SISC $k$-means seed & no & $42$ (released) \\
    GOOG / ZC=F downstream split & S\&P only & $62.5\%$ train (ours) \\
    Initial segment for generation & no & first test segment / random (ours) \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Protocol dependence of TATR}
...
\begin{figure}[t]
  ...
  \begin{subfigure}{0.49\linewidth}
    \centering
    \includegraphics[width=\linewidth]{02_tatr_protocol_contrast.pdf}
    \caption{Within-asset protocol contrast (S\&P 500).}
    \label{fig:tatr-protocol}
  \end{subfigure}\hfill
  \begin{subfigure}{0.49\linewidth}
    \centering
    \includegraphics[width=\linewidth]{07_cross_asset_nontransfer.pdf}
    \caption{Cross-asset non-transfer.}
    \label{fig:cross-asset}
  \end{subfigure}
  \caption{Downstream TATR depends on the synthetic-series protocol, and the S\&P 500 gain does not transfer. (a) On S\&P 500, continuous chunked (\texttt{split}) trajectories cross below the real-only baseline (dashed line marks the original 100-year TATR target; shaded bands show min/max across 30 seeds), while independent fixed blocks worsen as augmentation increases. (b) Change in downstream MAPE relative to each asset's no-augmentation baseline (below zero beats the real-only baseline): the \texttt{split} rollout helps only on S\&P 500 (best $-78\%$, final $-72\%$); on GOOG and ZC=F it never beats baseline and finishes $+92\%$ and $+179\%$. A protocol carrying genuine structure would not flip sign across assets.}
  \label{fig:downstream-tatr}
\end{figure}
...
FTS-Diffusion proposes a sophisticated architecture for financial time series: variable-length motifs, pattern-conditioned diffusion, and a learned motif chain. Working only from the code the authors released, we found that in several places the implementation does not match the paper. Its pattern-evolution module trains the duration as a classification although the paper specifies regression, and fits the magnitude with an $L_1$ loss rather than the $L_2$ the paper implies; the magnitude $\beta$ never enters the diffusion conditioning and is reapplied over a commented-out range normalization, so it loses its meaning; and generation rolls out the chain with \texttt{argmax} rather than the stochastic Markov transition the paper describes. Its scaling-AE decoder also ships with a single hidden unit.
