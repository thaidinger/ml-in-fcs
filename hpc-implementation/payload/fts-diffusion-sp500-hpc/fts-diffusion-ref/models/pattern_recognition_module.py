...
class SISC:
  ...
  def save_results(self, new_loss, dataname, init_strategy, barycenter):
    if new_loss<=self.min_loss:
      ...
      checkpoint_prefix = f"{filename}_{timestamp}"
      df_centroids.to_csv(res_path + checkpoint_prefix +'_centroids.csv')
      df_labels.to_csv(res_path + checkpoint_prefix + '_labels.csv')
      df_subsequences.to_csv(res_path + checkpoint_prefix + '_subsequences.csv')
      df_segmentation.to_csv(res_path + checkpoint_prefix + '_segmentation.csv')
      try:
        with open(res_path + filename + '.latest', 'w') as f:
          f.write(checkpoint_prefix)
      except Exception:
        pass
