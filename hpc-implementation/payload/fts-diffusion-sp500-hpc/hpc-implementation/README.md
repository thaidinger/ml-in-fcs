...
- `scripts/run_downstream_task.py`: one Slurm array task, one downstream run id.
...
- `slurm/apls_downstream_array.sbatch`: 100-array-job downstream run.
...
- 100 independent runs via a Slurm array (`--array=0-99`)
...
tail -f /iopsstor/scratch/cscs/$USER/fts-diffusion-sp500-hpc/logs/fts-sp500-downstream_<JOBID>_<ARRAYID>.out
```

Each completed array task writes:
